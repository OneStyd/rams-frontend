...
class LegerGerbangTolController extends Controller
{
    ...
    {
        $url = "http://117.53.47.111:91/api/leger/gerbang/{$request->jalan_tol_id}/{$request->leger_id_awal}/{$request->leger_id_akhir}";
    ...
    {
        $url = "http://117.53.47.111:91/api/leger/gerbang-all/{$request->jalan_tol_id}";
        $data = Http::withHeaders([
            'Accept' => 'application/json',
            'Authorization' => 'Bearer ' . $request->session()->get('token'),
        ])->get($url)->json();
